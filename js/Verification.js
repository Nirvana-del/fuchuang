$(function(){
  let verFault = document.getElementById("verFault");
  ...
  $("#ver").on('blur',function(){
   var val = $(".input-val").val().toLowerCase();
   var num = show_num.join("");   //用于把数组中的所有元素放入一个字符串。
   if(val==''){
    verFault.innerHTML="验证码不能为空";
    verFault.style.display="block";
    return false;
   }else if(val == num){
	verFault.innerHTML="验证码正确 √";
	verFault.style.color="green";
   }else{
    verFault.innerHTML="验证码错误";
    $(".input-val").val('');
    draw(show_num);
   }
  })
